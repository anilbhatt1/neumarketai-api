...
# Path to the YAML file (One directory up to lf_keywordgen, then into 'agent_yaml/reddit_resp_agent_yaml')
agent_yaml_path = os.path.join(current_dir, '..', 'agent_yaml', 'keyword_gen_ag.yaml')
with open(agent_yaml_path, 'r') as yaml_file:
    agent_cfg_data = yaml.safe_load(yaml_file)
    
# Path to the YAML file (One directory up to lf_keywordgen, then into 'agent_yaml/reddit_resp_task_yaml')
task_yaml_path = os.path.join(current_dir, '..', 'task_yaml', 'keyword_gen_tk.yaml')
